...
const TreeMapCustomizeBody = ({ chartDataArr, chartId, closeModal }: any) => {
  ...
  useEffect(() => {
    if (chartDataArr && chartId) {
      const chartdata = chartDataArr?.filter(
        (chartData: any) =>
          chartData.simpleTreeMapId === chartId ||
      ...
      const formData = {
        ...
        innerChartNameOne: chartdata[0]?.data?.length
          ? chartdata[0]?.data[0]?.name
          : "",
        innerChartNameSecond: chartdata[0]?.data?.length
          ? chartdata[0]?.data[1]?.name
          : "",
        outerChartNameOne: chartdata[0]?.data?.length
          ? chartdata[0]?.data[2]?.name
          : "",
        outerChartNameSecond: chartdata[0]?.data?.length
          ? chartdata[0]?.data[3]?.name
          : "",
      };
            ...
            if (chartId) {
              ...
              switch (filterData?.chartType) {
                case "Simple tree map":
                  url = `/company/simpleTree/map/${chartId}`;
                  break;
...
export default TreeMapCustomizeBody;
