...
import piChart from "../images/ComposedResponsiveContainer.png";
...
const ComposedResponsiveChart = (props: any) => {
  ...
  const getChartData = async () => {
    await getData("/company/composedResponsive/container", token)
      .then(function (resp) {
  ...
  const addData = async () => {
    await postDataWithToken(
      "/company/composedResponsive/container",
      {
        chartType: "Composed Responsive Container",
        chartLabel: "Combined Chart",
      },
      token
    )
      .then(function (resp) {
        ...
        if (chartData) {
          ...
          props?.handleEditChart(
            chartData?.data?.composedResponsiveContainerId,
  ...
  const deleteChart = async () => {
    ...
    await deleteData(`/company/composedResponsive/container/${chartId}`, token)
      .then(function (resp) {
        const chartData = resp?.data;
        console.log(chartData);
  ...
  const deleteKIM = async (chartId: string, kimId: string) => {
    await deleteData(
      `/company/composedResponsive/container/kim/${chartId}?kimId=${kimId}`,
      token
    )
      .then(function (resp) {
        const chartData = resp?.data;
            ...
            return (
              <div
                ...
                className={`m-2 chart-tile border hover:border-purple p-2 my-2 relative min-h-[208px] grow self-stretch max-w-[33.33%] ${
                  ...
                  chart?.composedResponsiveContainerId
                    ? "border-purple"
                    : "border-[#868686]"
                }`}
              >
                    ...
                    <div className="flex h-[auto] p-2 align-center ml-3">
                      <div
                        ...
                        className={`flex items-center p-1 mx-1 transition cursor-pointer chart-icons white-svg hover:bg-purple ${
                          props?.selectEditChart?.id ===
                            chart?.composedResponsiveContainerId && "bg-purple"
                        }`}
                      >
                        {editIcon}
                      </div>
                      ...
                      <div
                        onClick={() => {
                          setChartId(chart?.composedResponsiveContainerId);
                          setDeleteModal(true);
                        }}
                        className="flex items-center p-1 mx-1 transition cursor-pointer chart-icons white-svg hover:bg-purple"
                      >
